...
def word_filter_counter(text, filter_words):
    ...
    words = re.split(r"[\s.()-?!]+",text)#获取输入（文本与列表），文本格式处理
    filter_words = list(input("Please enter fliter words separated by space.").split())
    dic = dict.fromkeys(filter_words, 0)#以列表作为键创建字典
    for a in words:
        if a in dic:
            dic[a]=dic[a]+1#历遍文本
    print(dic)#输出字典
